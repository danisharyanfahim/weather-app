...
export default function WeeatherApp({ defaultCity = 'Toronto' }: { defaultCity: string }) {
  ...
  const search = async (cityName: string) => {
    ...
    if (weatherData.cod !== 200) { //If it returns anything other than status 200
      setFound(false)
    } else {
      setData(weatherData)
      if (!found) {
  ...
  return (
    <div className="weather-page">
      ...
      <input type="search" name="search" className="search-bar" id="search-bar" onKeyDown={handleKeyDown} onChange={handleChange} value={location} />
      <button className="search-button" onClick={() => search(location)} >
        <FaMagnifyingGlass />
      </button>
      </div>
        {found && data ? <WeatherCard {...data} /> : 'Not Found'}
    </div>
  )
}
